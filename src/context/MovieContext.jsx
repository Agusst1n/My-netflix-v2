...
const MoviesProvider = ({ children }) => {
  ...
  const SearchUrl = searchMovie
    ? `https://api.themoviedb.org/3/search/movie?query=${searchMovie}&${API_KEY}`
  ...
  const getGenres = async (genreID) => {
    ...
    try {
      ...
      const res = await fetch(
        `${BASE_URL}/discover/movie?${API_KEY}${MORE_DATA}`
      );
      const data = await res.json();
  ...
  const deleteFavouriteMovie = async (id) => {
    try {
      //Tengo que conseguir esa id
      const res = await fetch(
        `https://my-neftlix-default-rtdb.firebaseio.com/users/${user.localId}/${id}.json?auth=${user.idToken}`,
        {
          method: 'DELETE'
        }
      );
  ...
  const getSimilarMovies = async (id) => {
    try {
      const res = await fetch(
        `https://api.themoviedb.org/3/movie//${id}/similar?api_key=3d9d528c10bd10aab1dcbcd5f1f8f9bf&language=en-US&page=1`
      );
      const data = await res.json();
